...
pub fn app_root() -> PathBuf {
  tauri::api::path::home_dir().unwrap().join(".quickType")
}
...
pub fn script_path() -> PathBuf {
  ...
  if !exists(&script_file) {
    ...
    fs::write(
      &script_file,
      format!(
        "// *** QuickType User Script ***\n \n// @path: {}\n\nconsole.log('🤩 Hello QuickType');",
...
pub fn clear_conf(app: &tauri::AppHandle) {
  ...
  let msg = format!(
    "Path: {}\n
    Are you sure you want to clear all QuickType configurations? Performing this operation data can not be restored, please back up in advance.\n
...
pub async fn get_data(
  ...
  if is_ok {
    ...
    if let Some(v) = app {
      tauri::api::dialog::message(v.get_window("core").as_ref(), "QuickType HTTP", body);
    }
    Ok(None)
  }
}
